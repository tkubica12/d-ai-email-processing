...
class CosmosDBConfig(BaseModel):
    ...
    submissions_trigger_container_name: str = Field(
        ...,
        description="Cosmos DB submissions trigger container name",
        example="submissionstrigger"
    )
...
class AppConfig(BaseModel):
    ...
    def from_env(cls) -> 'AppConfig':
        ...
        cosmos_db = CosmosDBConfig(
            ...
            submissions_trigger_container_name=os.getenv("AZURE_COSMOS_DB_SUBMISSIONS_TRIGGER_CONTAINER_NAME")
        )
        
        # Azure OpenAI configuration (placeholder - not needed for submission-trigger)
        openai = AzureOpenAIConfig(
            endpoint=os.getenv("AZURE_OPENAI_ENDPOINT", "https://placeholder.openai.azure.com/"),
        ...
        table_storage = TableStorageConfig(
            ...
            enabled=os.getenv("AZURE_TABLE_STORAGE_ENABLED", "false").lower() == "true"
        )
