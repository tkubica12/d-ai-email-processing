resource "azapi_resource" "docproc_classifier" {
  ...
  body = {
    identity = {
      ...
      userAssignedIdentities = {
        "${azurerm_user_assigned_identity.docproc_classifier.id}" = {}
      }
    }
    properties = {
      ...
      template = {
        ...
        containers = [
          {
            name  = "docproc-classifier"
            image = "ghcr.io/${var.github_repository}/docproc-classifier:latest"
            resources = {
              cpu    = 0.5
              memory = "1Gi"
            }
            env = [
              {
                ...
                value = azurerm_user_assigned_identity.docproc_classifier.client_id
              },
              {
